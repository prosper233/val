...
button.addEventListener("click", () => {
    if (button.textContent === "Click Me! ❤") {
        ...
        fetch('send_mail.php', {
            ...
            body: 'email=' + encodeURIComponent(recipientEmail) + '&name=' + encodeURIComponent(recipientName)
        })
            .then(response => {
                if (response.ok) {
                    button.textContent = "Check Your Email 🙃";
                    const trafficMessage = document.getElementById("trafficMessage");
                    trafficMessage.textContent = "incase you no receive email no vex na traffic cause am";
                    trafficMessage.style.display = "block";
                } else {
...
function drawText() {
    ...
    // glow effect
    context.shadowColor = "rgba(233, 30, 99, 1)";
    context.shadowBlur = 8;
    context.shadowOffsetX = 0;
    context.shadowOffsetY = 0;

    if (frameNumber < 250) {
        context.fillStyle = `rgba(233, 30, 99, ${opacity})`;
    ...
    if (frameNumber > 2500 && frameNumber < 99999) {
        ...
        if (window.innerWidth < 600) {
            drawTextWithLineBreaks([`I love you so much ${recipientName}, more than`, "all the time and space in the universe can contain"], canvas.width / 2, canvas.height / 2, fontSize, lineHeight);
        } else {
            context.fillText(`I love you so much ${recipientName}, more than all the time and space in the universe can contain`, canvas.width / 2, canvas.height / 2);
        }
    ...
    if (frameNumber >= 2750 && frameNumber < 99999) {
        context.fillStyle = `rgba(233, 30, 99, ${secondOpacity})`;
    ...
    if (frameNumber >= 3000 && frameNumber < 99999) {
        context.fillStyle = `rgba(233, 30, 99, ${thirdOpacity})`;
...
function draw() {
    ...
    if (frameNumber < 99999 && isStarted) {
        frameNumber++;
    }
...
document.getElementById("startExperience").addEventListener("click", () => {
    ...
    const emailInput = document.getElementById("recipientEmail").value;
